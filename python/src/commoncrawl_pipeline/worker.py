...
from commoncrawl_pipeline.metrics import (
    worker_batch_item_download_errors,
    worker_batch_item_parse_errors,
    worker_batch_items_processed,
    worker_batch_items_total,
    worker_batch_processing_errors,
    worker_batches,
    worker_documents_extracted,
    worker_documents_extraction_failed,
    worker_documents_written,
    worker_download_bytes_total,
    worker_upload_bytes_total,
    worker_upload_errors,
    worker_warc_records_non_response,
    worker_warc_records_response,
    worker_warc_records_total,
)
...
def process_batch(
    ...
    try:
        ...
        worker_batch_items_total.inc(len(batch))
    except (json.JSONDecodeError, TypeError):
        logger.error("Failed to parse batch message", exc_info=True)
        worker_batch_processing_errors.inc()
    ...
    for item in batch:
        try:
            ...
            try:
                ...
                worker_download_bytes_total.inc(len(data))
                ...
                worker_batch_item_download_errors.inc()
                items_failed += 1
            ...
            try:
                for record in WARCIterator(io.BytesIO(data)):
                    worker_warc_records_total.inc()
                    if record.rec_type == "response":
                        worker_warc_records_response.inc()
                        try:
                            text = trafilatura.extract(record.content_stream().read())
                            if text is not None:
                                worker_documents_extracted.inc()
                                ...
                                try:
                                    ...
                                    worker_documents_written.inc()
                                    # Track approximate JSON size
                                    worker_upload_bytes_total.inc(
                                        len(json.dumps(document))
                                    )
                                except Exception:
                                    worker_upload_errors.inc()
                                    logger.error(
                                ...
                                worker_documents_extraction_failed.inc()
                        except Exception:
                            logger.error("Text extraction failed", exc_info=True)
                            worker_documents_extraction_failed.inc()
                            # Continue with next record
                    else:
                        worker_warc_records_non_response.inc()
            except Exception:
                logger.error("WARC processing failed", exc_info=True)
                worker_batch_item_parse_errors.inc()
                items_failed += 1
                continue

            worker_batch_items_processed.inc()
            items_processed += 1
